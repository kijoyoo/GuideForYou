    ...
    <body>
        ...
        <div class="container myPage_wrapper">
            ...
            <main class="guideReservationList_main">
                <div class="guideReservationList_main_title">가이드에게 리뷰주기</div>
                <div class="guideWritereview_main_title ">투어 가이드</div>
                
                
                <section id="guidelList_notcomp" class=" guideReservationList_notcomp board_tabcontent">
                  <c:forEach items="${customerGuideInfoTourListForWriteReview }" var="memberTourInfo">
                    <div class="writeReview_column task ">
                        <div class="title guidelList_content_title">${memberTourInfo.tourBasicVo.tour_intro }</div>
                        <div class="writeReview_column_div"></div>
                        <div class="guidelList_content_nick">
                          <c:choose >
                        	<c:when test="${memberTourInfo.memberBasicVo.member_profile_photo=='n'}">
                        		<a href="./credit_infoPage?member_idx=${memberTourInfo.memberBasicVo.member_idx }" class="guideReservation_photo_column ">
                                	<img src="myPageImg/basic_img.jpg" class="guideReservation_photo">
                            	</a>
                        	</c:when>
                        	<c:otherwise>
                            	<a href="./credit_infoPage?member_idx=${memberTourInfo.memberBasicVo.member_idx }" class="guideReservation_photo_column ">
                                	<img src="${memberTourInfo.memberBasicVo.member_profile_photo=='n'}" class="guideReservation_photo">
                            	</a>
                            </c:otherwise>	
                          </c:choose>
                            <div class="writeReview_column_name guidelList_content ">${memberTourInfo.memberBasicVo.member_nick}</div> 
                        </div> 
                        <div class="writeReview_column_div"></div>
                        <a href="./customerEvaluationWritePage?tour_idx=${memberTourInfo.tourBasicVo.tour_idx}" class="n_guide_arrow ">가이드 리뷰작성<i class="fas fa-chevron-right "></i></a>
                    </div>
                  </c:forEach>
